...
def verify_candidate(repo: pathlib.Path, base_commit: str, candidate_commit: str, bounty_id: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="motoko-issue-1-verifier-") as tmp:
        tmp_path = pathlib.Path(tmp)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Protected verifier for Motoko issue #1")
